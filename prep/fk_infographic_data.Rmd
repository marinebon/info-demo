...
library(devtools)
library(servr) #needed servr to get infographiq to install 
...
if(!all(file.exists("prep/data/rvc_trophic_groups/jacks_abun.csv",
  ...
  if(!all(file.exists(opportunists_fk_abun_csv,opportunists_fk_den_csv,opportunists_fk_bio_csv,opportunists_fk_diversity_csv))){
    ...
    opportunists_abundance = opportunists_abun %>%
      group_by(YEAR) %>%
      summarize(
        abundance_mean = mean(abundance),
        abundance_n = length(abundance),
        ...
        abundance_min = min(abundance),
        abundance_max = max(abundance))
    write_csv(opportunists_abundance, "prep/data/rvc_trophic_groups/opportunists_abun.csv")
...
if(!file.exists("prep/data/rvc_trophic_level/trophic_level_biomass.csv")){ 
  ...
  trophic_level_biomass = read_csv("prep/data/rvc_trophic_level/trophic_level_bio.csv")
  
  trophic_level = trophic_level_biomass %>%
    select(YEAR, biomass, SPECIES_CD) %>% 
    left_join(trophic_levels, trophic_level_biomass, by = 'SPECIES_CD')
  ...
  write_csv(trophic_level, "prep/data/rvc_trophic_level/trophic_level.csv")
...
if(!file.exists("prep/data/domain_div.csv")){
  ...
  #write_rds(domain_abun, "prep/data/domain_abun")
  
  domain_abun = read_rds("domain_abun")

  domain_div = domain_abun %>%
    filter(protected_status != "all") %>%
    select(YEAR, SPECIES_CD, abundance, protected_status) %>%
    group_by(YEAR, protected_status) %>%
    nest(-YEAR) %>%
    mutate(
      data_wide = map(data, ~ spread(data=.x, SPECIES_CD, abundance, fill =0)),
...
queen_conch = read_csv("prep/data/queen_conch/QueenConchData.csv") #1993-2013

toupper(names(queen_conch))
...
lobster = read_csv("prep/data/lobster/Lobster_landings.csv") #1991-2015

toupper(names(lobster))
#"Year","Commercial_lbs","Recreational_lbs","Total"  
...
|:----------------------------------------|:----------------------------------------------|:--------------------------------------------|
|*ACROPORA PALMATA*                       | elkhorn coral                                 |"ACROPORA.PALMATA"                           |
|*ACROPORA PROLIFERA*                     | fused staghorn coral                          |"ACROPORA.PROLIFERA"                         |
|*ACROPORA CERVICORNIS*                   | staghorn coral                                |"ACROPORA.CERVICORNIS"                       |
|*AGARICIA FRAGILIS*                      | leaf lettuce coral                            |"AGARICIA.FRAGILIS"                          |
|*AGARICIA LAMARCKI*                      | lamarckis sheet coral                         |"AGARICIA.LAMARCKI"                          |              
|*CLADOCORA ARBUSCULA*                    | tube coral                                    |"CLADOCORA.ARBUSCULA"                        |
|*COLPOPHYLLIA NATANS*                    | boulder brain coral/large-grooved brain coral |"COLPOPHYLLIA.NATANS"                        |
|*DENDROGYRA CYLINDRUS*                   | pillar coral                                  |"DENDROGYRA.CYLINDRUS"                       |  
|*DICHOCOENIA STOKESII*                   | pineapple coral/elliptical star coral         |"DICHOCOENIA.STOKESII"                       |        
|*DIPLORIA LABYRINTHIFORMIS*              | grooved brain coral                           |"DIPLORIA.LABYRINTHIFORMIS"                  | 
|*EUSMILIA FASTIGIATA*                    | smooth flower coral                           |"EUSMILIA.FASTIGIATA"                        |
|*FAVIA FRAGUM*                           | golfball coral                                |"FAVIA.FRAGUM"                               |     
|*HELIOSERIS.CUCULLATA*                   | sunray lettuce coral                          |"HELIOSERIS.CUCULLATA"                       |         
|*ISOPHYLLIA SINUOSA*                     | sinuous cactus coral                          |"ISOPHYLLIA.SINUOSA"                         |
|*ISOPHYLLIA RIGIDA*                      | rough star coral                              |"ISOPHYLLIA.RIGIDA"                          | 
|*MADRACIS DECACTIS.COMPLEX*              | NA                                            |"MADRACIS.DECACTIS.COMPLEX"                  | 
|*MADRACIS AURENTENRA*                    | Yellow pencil coral                           |"MADRACIS.AURENTENRA"                        |
|*MANICINA AREOLATA*                      | rose coral                                    |"MANICINA.AREOLATA"                          |         
|*MEANDRINA MEANDRITES*                   | maze coral                                    |"MEANDRINA.MEANDRITES"                       |            
|*MILLEPORA COMPLANATA*                   | blade fire coral                              |"MILLEPORA.COMPLANATA"                       |  
|*MILLEPORA ALCICORNIS*                   | sea ginger                                    |"MILLEPORA.ALCICORNIS"                       | 
|*MONTASTRAEA CAVERNOSA*                  | great star coral                              |"MONTASTRAEA.CAVERNOSA"                      |           
|*MUSSA ANGULOSA*                         | spiny flower coral/large flower coral         |"MUSSA.ANGULOSA"                             |
|*MYCETOPHYLLIA LAMARCKIANA.COMPLEX*      | NA                                            |"MYCETOPHYLLIA.LAMARCKIANA.COMPLEX"          |
|*MYCETOPHYLLIA FEROX*                    | Rough Cactus Coral                            |"MYCETOPHYLLIA.FEROX"                        |       
|*MYCETOPHYLLIA ALICIAE*                  | Knobby Cactus Coral                           |"MYCETOPHYLLIA.ALICIAE"                      |
|*OCULINA ROBUSTA*                        | Ivory tree coral                              |"OCULINA.ROBUSTA"                            |      
|*OCULINA DIFFUSA*                        | Ivory Bush Coral                              |"OCULINA.DIFFUSA"                            |
|*ORBICELLA ANNULARIS.COMPLEX*            | NA                                            |"ORBICELLA.ANNULARIS.COMPLEX"                |
|*PORITES ASTREOIDES*                     | mustard hill coral                            |"PORITES.ASTREOIDES"                         |
|*PORITES PORITES COMPLEX*                | NA                                            |"PORITES.PORITES.COMPLEX"                    |
|*PSEUDODIPLORIA STRIGOSA*                | symmetrical brain coral                       |"PSEUDODIPLORIA.STRIGOSA"                    |
|*PSEUDODIPLORIA CLIVOSA*                 | knobby brain coral                            |"PSEUDODIPLORIA.CLIVOSA"                     |
|*SCLERACTINIA SP*                        | NA                                            |"SCLERACTINIA"                               |
|*SCOLYMIA SP*                            | NA                                            |"SCOLYMIA.SP_"                               |
|*SIDERASTREA SIDEREA*                    | massive starlet coral/round starlet coral     |"SIDERASTREA.SIDEREA"                        |
|*SIDERASTREA RADIANS*                    | lesser starlet coral/shallow-water starlet coral|"SIDERASTREA.RADIANS"                      |
|*SOLENASTREA BOURNONI*                   | smooth star coral                             |"SOLENASTREA.BOURNONI"                       |
|*STEPHANOCOENIA INTERSEPTA*              | blushing star coral                           |"STEPHANOCOENIA.INTERSEPTA"                  |
|*UNDARIA AGARICITES COMPLEX*             | NA                                            |"UNDARIA.AGARICITES.COMPLEX"                 |


```{r foundation species}

if(!file.exists("coral_div.csv")){
  
  ##Megan: working off Luke's code 
  sc_pcov_spp= read_csv("prep/data/CREMP/CREMP_Pcover_spp_tpose.csv") #20*41
  toupper(names(sc_pcov_spp))
  # "YEAR"                              "PSEUDODIPLORIA.STRIGOSA"           "UNDARIA.AGARICITES.COMPLEX"       
  # "STEPHANOCOENIA.INTERSEPTA"         "SOLENASTREA.BOURNONI"              "SIDERASTREA.SIDEREA"              
  # "SIDERASTREA.RADIANS"               "SCOLYMIA.SP_"                      "SCLERACTINIA"                     
  # "PORITES.ASTREOIDES"                "PSEUDODIPLORIA.CLIVOSA"            "PORITES.PORITES.COMPLEX"          
  # "ISOPHYLLIA.RIGIDA"                 "ORBICELLA.ANNULARIS.COMPLEX"       "OCULINA.ROBUSTA"                  
  # "OCULINA.DIFFUSA"                   "MYCETOPHYLLIA.LAMARCKIANA.COMPLEX" "MYCETOPHYLLIA.FEROX"              
  # "MYCETOPHYLLIA.ALICIAE"             "MONTASTRAEA.CAVERNOSA"             "MUSSA.ANGULOSA"                   
  # "MEANDRINA.MEANDRITES"              "MILLEPORA.COMPLANATA"              "MILLEPORA.ALCICORNIS"             
  # "MADRACIS.DECACTIS.COMPLEX"         "MANICINA.AREOLATA"                 "MADRACIS.AURENTENRA"              
  # "ISOPHYLLIA.SINUOSA"                "ACROPORA.CERVICORNIS"              "HELIOSERIS.CUCULLATA"             
  # "ACROPORA.PALMATA"                  "ACROPORA.PROLIFERA"                "AGARICIA.FRAGILIS"                
  # "AGARICIA.LAMARCKI"                 "CLADOCORA.ARBUSCULA"               "DENDROGYRA.CYLINDRUS"             
  # "COLPOPHYLLIA.NATANS"               "DICHOCOENIA.STOKESII"              "DIPLORIA.LABYRINTHIFORMIS"        
  # "FAVIA.FRAGUM"                      "EUSMILIA.FASTIGIATA"  
  ...
  coral_diversity = sc_pcov_spp %>%
    gather(key=CORAL_SPP, value=measurement, -Year) %>%
    group_by(Year) %>%
    nest(-Year) %>%
    mutate(
      data_wide = map(data, ~ spread(data=.x, CORAL_SPP, measurement, fill =0)),
      richness = map(data_wide, function(x) specnumber(x)),
      simpson  = map(data_wide, function(x) 1/(1 - diversity(x, index = 'simpson'))),
      shannon = map(data_wide,  function(x) exp(diversity(x,  index = 'shannon')))) %>% 
    unnest(richness, simpson, shannon)
  
  coral_div = coral_diversity %>%
    select(Year, richness, simpson, shannon)
  
  write_csv(coral_div, "prep/data/coral_div.csv")
  
  cremp_abund = read_csv("prep/data/CREMP/CREMP_Pcover_group.csv") #1996-2015
  ...
  crichness = read_csv("prep/data/CREMP/CREMP_Domain_Coral_Richness.csv") #1996-2015
  toupper(names(crichness)) #"ROW LABELS","DOMAIN RICHNESS"
}

```

```{r practice coral infographic}

test1 = infographiq::create_info_site(path_root = ".",
                 #dir_svg = "",
                 elements_csv = "svg_elements.csv",
                 indicators_csv = "plot_indicators.csv")

plot_indicators = read_csv("plot_indicators.csv", 
                           title   = 'Shark Richness',
                           y_label = 'Effective number of species'))
#Parsed with column specification:
#cols(
  # Classification_level = col_character(),
  # svg_id = col_character(),
  # plot_title = col_character(),
  # filter = col_character(),
  # col_t = col_character(),
  # col_y = col_character(),
  # col_se = col_character(),
  # y_label = col_character(),
  # group_by = col_character(),
  # csv_url = col_character(),
  # direct_path = col_character())

plot_timeseries("https://github.com/marinebon/info-fk/blob/master/prep/data/rvc_trophic_groups/sharks_div.csv")
